...
$db->real_connect($host, $username, $password, $dbname);
// 检测连接
if ($db->connect_error) {
	...
	$db->query("set names utf8"); 
            	...
            	$create_sql = array(
            		...
            		"CREATE TABLE `mxgapi_friendlink_apply` (`id` int(11) NOT NULL,`name` varchar(255) NOT NULL,`url` varchar(255) NOT NULL,`picurl` varchar(255) NOT NULL,`desc` varchar(255) NOT NULL,`contact` varchar(100) NOT NULL DEFAULT '',`ip` varchar(50) NOT NULL DEFAULT '',`time` int(20) NOT NULL,`status` tinyint(1) NOT NULL DEFAULT '0',`remark` varchar(255) NOT NULL DEFAULT '') ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='友情链接申请表';",
		...
		"INSERT INTO `mxgapi_plan` (`name`,`description`,`price`,`duration`,`level`,`features`,`sort`,`status`,`create_time`) VALUES('永久会员','终身无限次API调用','100.00',0,2,'{\"calls\":\"999999\",\"discount\":\"100%\",\"support\":\"priority\"}',3,1,'".time()."');"
            	...
            	$alter_sql = array(
					...
					"ALTER TABLE `mxgapi_friendlink_apply` ADD PRIMARY KEY (`id`);",
            	...
            	$alter2_sql = array(
					...
					'ALTER TABLE `mxgapi_friendlink_apply` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=0;',
            	...
            	$plan_result = loop_query($db, $plan_sql);
            	$alter_result = loop_query($db, $alter_sql);
				$alter2_result = loop_query($db, $alter2_sql);
            	if($init_result && $create_result && $insert_result && $plan_result && $alter_result && $alter2_result){
					...
					unlink('./Database/connect.php');
					$db->query('DROP TABLE `mxgapi_access`, `mxgapi_api`, `mxgapi_config`, `mxgapi_feedback`, `mxgapi_friendlinks`, `mxgapi_friendlink_apply`, `mxgapi_login_log`, `mxgapi_post`, `mxgapi_spider`, `mxgapi_user`, `mxgapi_plan`, `mxgapi_order`;');
