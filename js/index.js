...
if ($('#loading').length) {
    loadingDialog = new mdui.Dialog('#loading', {
        history: false,
        modal: true,
        closeOnEsc: false,
    });
...
$(document).ready(function() {
    ...
    const currentAction = new URLSearchParams(path).get('action') || 'home';
    
    // 桌面端导航激活
    document.querySelectorAll('.new-nav-link').forEach(link => {
        const href = link.getAttribute('href');
        const matchAction = new URLSearchParams(href.replace('?', '')).get('action') || 'home';
        if (matchAction === currentAction || (currentAction === '' && matchAction === 'home')) {
